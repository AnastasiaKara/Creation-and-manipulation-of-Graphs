...
def answer_one():
    # Your Code Here
    A1 = nx.Graph()
    A1.add_nodes_from(employees, bipartite=0)  # label one set of nodes 0
    A1.add_nodes_from(movies, bipartite=1)  # label other set of nodes 1
    first_line = True
    file =  open('employee_movies_preferences.txt', 'r') #in order not to write the whole directory as path (too big), I placed all the folders in the same file.
    for line in file:
        if first_line:
        ...
        node1 = temp[0]
        node2 = temp[1]
        A1.add_edge(node1, node2)
    file.close()

    return  A1 # Your Answer Here
...
def answer_two(G):
    ...
    for node in nx.nodes(G):
    	if node in employees:
    		attributes[node] = {'type':'employee'}
    	elif node in movies:
    		attributes[node] = {'type':'movie'}
    nx.set_node_attributes(G, attributes)
#    for node in nx.nodes(G):
#    	print(node['type'])
    nx.get_node_attributes(A2, 'type')     #prints attributes of all nodes

    return  A2 # Your Answer Here
