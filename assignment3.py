...
def answer_six(G2, dampingFactor):
    # Your Code Here
    import collections
    dampingFactor = 0.85
    Top5blogs = []
    pRank = nx.pagerank(G2,alpha=dampingFactor)
    Highest5 = dict(collections.Counter(pRank).most_common(5))
    for key, value in Highest5.items():
        temp = [key,value]
        Top5blogs.append(temp)

    Top5blogs.sort(key = lambda x: x[1], reverse = True)
    
    return  Top5blogs # Your Answer Here
...
def answer_seven(G2):
    # Your Code Here
    h,a = nx.hits(G2)
    hub = h.get('realclearpolitics.com')
    authority = a.get('realclearpolitics.com')
    t = (authority, hub)
    
    return  t # Your Answer Here
...
def answer_eight():
    ...
    h,a = nx.hits(G2)
    Top5hubs = []
    Highest5 = dict(Counter(h).most_common(5))
    for key, value in Highest5.items():
        temp = [key,value]
        Top5hubs.append(temp)
        
    Top5hubs.sort(key = lambda x: x[1], reverse = True)
    
    return Top5hubs  # Your Answer Here
...
def answer_nine():
    ...
    h,a = nx.hits(G2)
    Top5authorities = []
    Highest5 = dict(Counter(a).most_common(5))
    for key, value in Highest5.items():
        temp = [key,value]
        Top5authorities.append(temp)
        
    Top5authorities.sort(key = lambda x: x[1], reverse = True)
